...
# 디버그 출력 제어 플래그 (payload, 서버 응답 등 상세 로그)
DEBUG_PAYLOAD = False
...
def main():
    ...
    if use_any_perturb:
        ...
        # ood_task_configs의 상대 경로를 프로젝트 루트 기준 절대 경로로 변환
        if "ood_task_configs" in eval_cfg:
            ood_configs = eval_cfg["ood_task_configs"]
            for key, rel_path in ood_configs.items():
                if rel_path and not os.path.isabs(rel_path):
                    # 상대 경로인 경우 프로젝트 루트 기준으로 변환
                    abs_path = os.path.join(project_root, rel_path.lstrip("./"))
                    ood_configs[key] = os.path.normpath(abs_path)
                    print(f"[run_env] {key} config 경로 변환: {rel_path} -> {ood_configs[key]}")
    ...
    DEFAULT_INSTRUCTION = "put the red block on the blue plate"
    instruction = DEFAULT_INSTRUCTION  # 기본 instruction으로 초기화

    # 첫 관측 및 렌더링
    obs = env.reset()
    env.env.render()  # 초기 화면 렌더링
    print(
        "\n[run_env] 환경이 초기화되었습니다. "
        f"기본 instruction: '{DEFAULT_INSTRUCTION}'"
    )
    print("[run_env] 화면을 확인하신 후 instruction을 입력하거나, 엔터만 누르면 기본/이전 instruction을 사용합니다.")
    ...
    while True:
        # 버퍼가 비어 있으면 사용자로부터 instruction을 입력받고 서버에 새 action 시퀀스를 요청
        if len(action_buffer) == 0:
            # 사용자로부터 instruction 입력 받기
            print("\n" + "="*60)
            user_input = input("Instruction을 입력하세요 (엔터만 누르면 이전 instruction 재사용, 'quit' 입력 시 종료): ").strip()
            
            if user_input.lower() == 'quit':
                print("[run_env] 사용자가 종료를 요청했습니다.")
                break
            
            if user_input:
                instruction = user_input
                print(f"[run_env] 새로운 instruction: '{instruction}'")
            else:
                # 엔터만 누른 경우: 기본/이전 instruction 사용
                print(f"[run_env] 기본/이전 instruction 사용: '{instruction}'")
        ...
        if done:
            print("\n[run_env] Task가 완료되었습니다.")
            while True:
                choice = input(
                    "[run_env] 다시 시작하려면 'r', 종료하려면 'q'를 입력하세요 (엔터=다시 시작): "
                ).strip().lower()

                if choice in ("", "r"):
                    obs = env.reset()
                    env.env.render()  # 리셋 후 화면 렌더링
                    print("[run_env] 환경이 리셋되었습니다. 화면을 확인한 뒤 instruction을 입력해주세요.")
                    action_buffer = []  # 버퍼 초기화하여 다음 루프에서 사용자 입력 받기
                    break
                if choice == "q":
                    print("[run_env] 사용자가 종료를 선택했습니다. 프로그램을 종료합니다.")
                    return

                print("입력이 올바르지 않습니다. 'r' 또는 'q'를 입력해 주세요.")
