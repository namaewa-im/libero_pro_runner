...
Please install LIBERO-PRO first (https://github.com/Zxy-MLlab/LIBERO-PRO)
...
2. Start the model server from `openvla-oft` (python vla-scripts/deploy.py in openvla-oft repo.).
3. Run this client to send images and language instructions to the server and
   receive predicted actions via HTTP.
4. `use_text` triggers one model call per Enter key press, and the model
returns 8 actions by default.  
If you want continuous actions, simply keep pressing Enter repeatedly.
